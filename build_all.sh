...
<<<<<<< HEAD
    ...
    then
        ...
        go build -ldflags="-s -w" -o ./bin/mantle-v$TAG-$GOOS-armv$GOARM
    elif [ $GOARCH = 'arm64' ]
    then
        export GOARM=7
        echo $TAG-$GOOS-arm64v$GOARM
        go build -ldflags="-s -w" -o ./bin/mantle-v$TAG-$GOOS-arm64v$GOARM
    else
...
=======
    ...
    echo $tag-$GOOS-$GOARCH
    go build -ldflags="-s -w" -o ./bin/mantle-$tag-$GOOS-$GOARCH$ext
>>>>>>> upstream/master
}
